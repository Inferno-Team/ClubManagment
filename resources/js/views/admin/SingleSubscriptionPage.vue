<template>
    <div>
        <div v-if="isLoading" class="my-container">
            <md-progress-spinner class="spinner" :md-diameter="100" :md-stroke="5" md-mode="indeterminate" />
        </div>
        <div v-else>
                ...
                <md-table class="md-layout-item md-elevation-2 pt-3" v-model="paginatedCustomers" md-card md-sort="id"
                    ...
                    <md-table-toolbar>
                        <div class="md-toolbar-section-start">
                            <h1 class="md-title">New Clubs</h1>
                        </div>
                    ...
                    <md-table-row slot="md-table-row" slot-scope="{ item }">
                        ...
                        <md-table-cell md-label="Club Name" md-sort-by="club.name">
                        ...
                        <md-table-cell md-label="Customers Count" md-sort-by="user_subscriptions_count">{{ item.user_subscriptions_count }}</md-table-cell>
                    ...
                    <md-table-pagination :md-page-size="10" :md-data="customers"
                        :md-paginated-data.sync="paginatedCustomers" />

                </md-table>
                <md-dialog-confirm :md-active.sync="remove_subscription_dialog_active" md-title="Removing Subscription"
                    :md-content="`Do you really want to remove subscription of <strong>${remove_subscription.club.name}</strong> ?`"
...
export default {
    ...
    methods: {
        searchByName(items, term) {
            if (term) {
                return items.filter(item => this.toLower(item.club.name).includes(this.toLower(term)))
            }
        ...
        createCustomerChart() {
            this.customersCountChart.data.datasets[0].data
                = [
                    this.sub.number_of_customer_sub,
                    this.sub.number_of_last_month_subs,
                ];

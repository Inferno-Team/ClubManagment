<template>
    <div>
        ...
        <md-table v-else v-model="searched" md-card md-fixed-header style="height: 100vh;">
            ...
            <md-table-empty-state md-label="No subscriptions found"
                ...
                <md-button @click="showAddNewSubscriptionModal" class="md-primary md-raised">Create New Subscription</md-button>
            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }">
                ...
                <md-table-cell md-label="Number Of Clubs">{{ item.number_of_clubs }}</md-table-cell>
                <md-table-cell md-label="Number Of Subscriptions">{{ item.number_of_subs }}</md-table-cell>
        ...
        <md-dialog-confirm :md-active.sync="remove_sub_dialog_active" md-title="Removing Subscription Type"
            :md-content="`Do you really want to remove <strong>${remove_sub.name}</strong> ?`" md-confirm-text="Yes"
            md-cancel-text="No" @md-cancel="() => remove_sub = {}" @md-confirm="onSubRemoveConfirm" />
    </div>
</template>
...
export default {
    ...
    methods: {
        getAllSubscriptions() {
            this.isLoading = true;
            axios.get('/subscription/api/all')
                .then((response) => {
